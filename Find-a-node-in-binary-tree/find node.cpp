...
bool findNode(binaryTreeNode<int>* root,int x){
    ...
    bool isLeft=findNode(root->left,x);
    if(isLeft==true){
        return true;
    }
    bool isRight=findNode(root->right,x);
    if(isRight==true){
        return true;
    }
    return false;
...
int main(){
    ...
    printLevelWise(root);
    int x;
    cout<<"Enter the node data you want to find in a tree"<<endl;
    cin>>x;
    bool getNode=findNode(root,x);
    if(getNode==true){
        cout<<"True"<<endl;
